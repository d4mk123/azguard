...
def _check_internet_exposed(
    ...
    for nsg in nsgs:
        ...
        for rule in nsg.security_rules:
            if (
                rule.direction == SecurityDirection.INBOUND
                and rule.access == SecurityAccess.ALLOW
                and rule.protocol in [protocol, SecurityProtocol.ASTERISK]
                and _is_internet(rule.source_address_prefix)
                and _ports_intersect(rule.destination_port_range, target_ports)
            ):
                results.append(
                    CheckResult(
...
def check_udp_internet(nsgs):
    ...
    return _check_internet_exposed(
        nsgs,
        {53, 123, 161, 389, 1900},
        SecurityProtocol.UDP,
        "7.3",
        severity,
        "Sensitive UDP ports exposed to Internet",
    )
    ...
    for nsg in nsgs:
        ...
        for rule in nsg.security_rules:
            if (
                rule.direction == SecurityDirection.INBOUND
                and rule.access == SecurityAccess.ALLOW
                and rule.protocol == SecurityProtocol.ASTERISK
...
def check_missing_deny_all(nsgs):
    ...
    for nsg in nsgs:
        ...
        for rule in nsg.security_rules:
            if (
                rule.direction == SecurityDirection.INBOUND
                and rule.access == SecurityAccess.DENY
                and rule.protocol == SecurityProtocol.ASTERISK
                and rule.destination_port_range == "*"
                and rule.source_address_prefix == "*"
            ):
...
def check_overly_broad_service_tags(
    nsgs: list[NetworkSecurityGroup],
) -> list[CheckResult]:
    results: list[CheckResult] = []
    for nsg in nsgs:
        ...
        for rule in nsg.security_rules:
            if not (
                rule.access == SecurityAccess.ALLOW
                and rule.direction == SecurityDirection.INBOUND
            ):
                continue
            ...
            if rule.protocol == SecurityProtocol.ASTERISK:
                ...
                severity = _compute_severity(
                    1,
                    True,
                    _port_sensitivity(rule.destination_port_range, rule.protocol),
                )
                results.append(
                    CheckResult(
                        control_id="N/A",
...
def check_nsg_flow_logs_to_log_analytics(
    nsgs: list[NetworkSecurityGroup],
) -> list[CheckResult]:
    return [
        CheckResult(
...
def check_vnet_flow_logs_to_log_analytics(
    nsgs: list[NetworkSecurityGroup],
) -> list[CheckResult]:
    return [
        CheckResult(
...
def check_alert_create_update_nsg(
    nsgs: list[NetworkSecurityGroup],
) -> list[CheckResult]:
    return [
        CheckResult(
...
def check_network_watcher_enabled(
    nsgs: list[NetworkSecurityGroup],
) -> list[CheckResult]:
    return [
        CheckResult(
...
def check_network_security_perimeter(
    nsgs: list[NetworkSecurityGroup],
) -> list[CheckResult]:
    return [
        CheckResult(
...
def check_nsg_flow_log_retention(
    nsgs: list[NetworkSecurityGroup], flow_logs: list[FlowLog]
) -> list[CheckResult]:
    results: list[CheckResult] = []
    for nsg in nsgs:
...
def check_vnet_flow_log_retention(
    nsgs: list[NetworkSecurityGroup], flow_logs: list[FlowLog]
) -> list[CheckResult]:
    results: list[CheckResult] = []
...
def run_engine(
    nsgs: list[NetworkSecurityGroup], flow_logs: list[FlowLog] | None = None
) -> list[CheckResult]:
    results = []
