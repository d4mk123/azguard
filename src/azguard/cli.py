...
@app.command()
def scan(
    input_path: Path | None = typer.Option(None, "--input", "-i", help="Path to az network nsg list JSON export"),
    subscription: str | None = typer.Option(None, "--subscription", "-s", help="Azure subscription ID"),
    flow_logs: Path | None = typer.Option(None, "--flow-logs", help="Path to flow-log JSON export"),
    ...
    model: str = typer.Option("qwen2.5:7b-instruct", "--model", help="Ollama model name"),
):
    if input_path and subscription:
        typer.echo("Error: --input and --subscription are mutually exclusive.", err=True)
        raise typer.Exit(code=1)
    if not input_path and not subscription:
        typer.echo("Error: provide either --input <file> or --subscription <id>.", err=True)
        raise typer.Exit(code=1)
    ...
    nsgs = collect_from_file(input_path) if input_path else collect_from_azure(subscription)
    flow_logs_data = collect_flow_logs_from_file(flow_logs) if flow_logs else None
    ...
    if output:
        ...
        if report_format in ("html", "both"):
            out_html = output.with_suffix(".html") if output.suffix == ".pdf" else output

    if report_format in ("html", "both"):
        typer.echo(f"HTML report: {generate_html_report(results, narrative, nsgs, out_html)}")
    if report_format in ("pdf", "both"):
        typer.echo(f"PDF report:  {generate_pdf_report(results, narrative, nsgs, out_pdf)}")
    ...
    typer.echo(f"  Severity: Critical={sev['Critical']} High={sev['High']} Medium={sev['Medium']} Low={sev['Low']}")
