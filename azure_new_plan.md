...
3. **Virtual Network flow logs** (formerly NSG Flow Logs) — note: NSG Flow Logs are on a deprecation path (no new ones creatable after June 30, 2025; full retirement Sept 30, 2027). Build your "flow logging enabled" CIS check against **Virtual Network flow logs**, the current recommended replacement, not the legacy NSG flow log resource.
...
- Build `rule_engine.py` with check functions for the first 8–10 controls (internet-exposed admin ports, `Any` source rules, missing deny-all, etc.)
...
- **Deliverable:** `anomaly_detector.py` + a short write-up of the approach/limitations (small-n caveat matters — be upfront about it)
...
- **Deliverable:** `llm_report_writer.py`, tested against sample findings
